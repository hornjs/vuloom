...
export function createVirtualServerRoutesModule(result: ScannedRoutesResult): string {
  ...
  const uniqueFiles = Array.from(
    new Set(routeEntries.flatMap((route) => [route.file, ...route.middleware])),
  );
  ...
  return [
    ...
    `export const serverRoutes = ${serializeArrayOfObjects(
      routeEntries.map((route) => [
        ...
        [
          "middleware",
          `resolveInheritedMiddlewareNames(${serializeArray(route.middleware)})`,
        ],
      ]),
    )}`,
    "",
    "function resolveInheritedMiddlewareNames(files) {",
    ...
    "  }",
    "",
    '  if (!Array.isArray(middleware) || middleware.some((entry) => typeof entry !== "string" && typeof entry !== "function")) {',
    '    throw new Error(`Invalid server route middleware "${file}". Expected a default export or named "middleware" export with an array of middleware names and/or functions.`)',
    "  }",
    "",
    "  return middleware",
    "}",
    ...
    "  }",
    "",
    '  if (!Array.isArray(value) || value.some((entry) => typeof entry !== "string" && typeof entry !== "function")) {',
...
export function createVirtualServerPluginModule(): string {
  return [
    ...
    "    }",
    "",
    "    const middleware = registry[entry]",
    "    if (!middleware) {",
    '      throw new Error(`Unknown server middleware "${entry}" referenced by route "${route.id}".`)',
    "    }",
    "",
    "    return middleware",
    "  })",
