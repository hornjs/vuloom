...
export async function setupContentHooks(nuxt: Nuxt, siteConfig: SiteConfig) {
  let automdTransform: (content: string, path: string) => string | Promise<string> | undefined;
  if (siteConfig.automd) {
    const automd = await import("automd");
    const config = await automd.loadConfig(siteConfig.dir, siteConfig.automd);
