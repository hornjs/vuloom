...
type LandingConfig = Exclude<SiteConfig["landing"], false | undefined>;

const appConfig = useAppConfig();

const siteConfig = appConfig.site as SiteConfig;

const landing: LandingConfig & { _github: string } = defu(siteConfig.landing || {}, {
  // Meta
  navigation: false,
  ...
  heroSubtitle: siteConfig.shortDescription,
...
<template>
  <div>
    ...
    <UPageHero v-if="hero" :orientation="hero.orientation" class="relative" :links="hero.links">
      ...
      <template #title>
        {{ landing.heroTitle }}<br /><span
          ...
          class="text-primary text-4xl inline-block mt-4"
    ...
    <PageSponsors v-if="siteConfig.sponsors?.api" />
    <PageContributors v-if="siteConfig.landing?.contributors" />
  </div>
</template>
